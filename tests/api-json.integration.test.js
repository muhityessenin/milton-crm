...
test("full HTTP API preserves CRM behavior through JSON storage", async (t) => {
  ...
  const managerFree=(await call("GET","/api/slots?closerId=usr_closer")).body.find(item=>item.status==="FREE"),managerFreeDate=dayFrom(managerFree.startAt),summaryBefore=await call("GET",`/api/availability-summary?date=${managerFreeDate}`);assert.equal(summaryBefore.response.status,200);assert.equal(Object.keys(summaryBefore.body.items[0]).some(key=>/closer/i.test(key)),false);result=await call("POST","/api/clients",{name:"Manager view-only registration",phone:"+77015554991",closerId:"usr_closer",slotId:managerFree.id,assignmentMode:"NOW",preferredDate:managerFreeDate,preferredTimeText:"10:00",trialType:"FREE"});assert.equal(result.response.status,201);const managerOnlyClient=result.body;assert.equal(managerOnlyClient.activeTrial.assignmentState,"UNASSIGNED");assert.equal(managerOnlyClient.activeTrial.slotId,null);assert.equal((await call("GET","/api/slots?closerId=usr_closer")).body.find(item=>item.id===managerFree.id).status,"FREE");assert.equal((await call("GET","/api/unassigned-trials")).response.status,403);token=ownerToken;assert.equal((await call("DELETE",`/api/clients/${managerOnlyClient.id}`,{confirmation:"УДАЛИТЬ"})).response.status,200);token=closerToken;
