...
const statuses = [
  { id: "scheduled", name: "Запланирован", actionType: "NONE", sortOrder: 1, active: true, color: "#3157d5" },
...
test("only neutral active trials use Today and Planned columns", () => {
  assert.equal(views.boardColumnKey(client(), statuses, "2026-09-14"), views.TODAY);
  assert.equal(views.boardColumnKey(client({ activeTrial: { active: true, statusAtBookingId: "scheduled", preferredDate: "2026-09-15" } }), statuses, "2026-09-14"), views.PLANNED);
...
test("paid trial amount never impersonates a main-course payment", () => {
  ...
  assert.equal(views.boardColumnKey(paidTrial, statuses, "2026-09-14"), views.PLANNED);
});
...
test("every configured status remains a visible board column", () => {
  ...
  assert.deepEqual(columns.map((column) => column.key), [views.TODAY, views.PLANNED, ...configured.map((status) => `status:${status.id}`)]);
});
...
test("client grouping supports status, manager, closer, date and source", () => {
  ...
  assert.equal(views.groupClients([item], "status", statuses)[0].title, "Запланирован");
