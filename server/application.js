...
function enrichClient(c,viewer=null) {
  ...
  return { ...c, manager: publicUser(user(c.currentManagerId)), closer: publicUser(user(c.currentCloserId))||{id:null,name:"Без клоузера",role:"CLOSER",avatarUrl:""}, originalManager: publicUser(user(c.originalManagerId)), archivedBy:publicUser(user(c.archivedByUserId)),archiveReasonLabel:CLIENT_ARCHIVE_REASONS[c.archiveReason]||c.archiveReason||null,status: status(c.currentStatusId), currentReason:db.refusalReasons.find((x)=>x.id===c.currentReasonId)||null, leadSource: db.leadSources.find((x) => x.id === c.leadSourceId), tags: db.tags.filter((x) => c.tagIds.includes(x.id)), activeTrial, payments: allPayments, paymentTotal: allPayments.reduce((n, p) => n + Number(p.amount), 0), overdue: Boolean(overdue) };
}
...
async function api(req, res, url) {
  ...
  if (req.method === "POST" && /^\/api\/clients\/[^/]+\/status$/.test(url.pathname)) {
    ...
    const requiredFields=next.actionType==="REQUIRE_RESCHEDULE"?(input.rescheduleMode==="LATER"?["refusalReasonId"]:["refusalReasonId","newSlotId"]):(next.requiredFields||[]);const missing = requiredFields.filter((f) => input[f] === undefined || input[f] === null || input[f] === ""); if (missing.length) return fail(res, 422, "Заполните все обязательные поля", missing);
