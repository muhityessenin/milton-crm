...
function ensureOperationalNotifications(actor){
  ...
  const threshold=Number(db.meta.unassignedTrialReminderMinutes??120)*60000;if(threshold>0)for(const trial of db.trials.filter((item)=>canSeeUnassigned(actor,item)&&Date.now()-new Date(item.createdAt).getTime()>=threshold)){const c=client(trial.clientId),type="UNASSIGNED_TRIAL_OVERDUE";if(!db.notifications.some((n)=>n.userId===actor.id&&n.type===type&&n.trialId===trial.id))db.notifications.push({id:id("notif"),userId:actor.id,clientId:c.id,trialId:trial.id,type,content:`${c.name} · пробный ожидает назначения`,readAt:null,createdAt:now()});}
}
...
async function api(req, res, url) {
  ...
  if (req.method === "POST" && url.pathname === "/api/clients") {
    ...
    const slot=unassigned?null:db.availabilitySlots.find((s)=>s.id===input.slotId&&s.closerId===input.closerId);if(!unassigned&&(!slot||slot.status!=="FREE"))return fail(res,409,"Это время больше недоступно");
