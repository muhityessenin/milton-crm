...
const { financeReport } = require("./finance");
...
const defaultRussianNames = { st_today:"Сегодняшние", st_scheduled:"Запланированные", st_completed:"Завершён", st_reschedule:"Перенесён", st_no_show:"Неявка", st_refusal:"Отказ", st_payment:"Оплата", src_1:"Instagram", src_2:"TikTok", src_3:"Рекомендация", src_4:"Школа" };
const PERMISSION_CATALOG = {
  ...
  payments: ["view","create","edit","delete","viewHistory"],
  ...
  audit: ["view"]
  ,finance: ["own","team","allManagers","allClosers","companyTurnover","allPayments","allTrials","clientHistory","export","manageRates","manageTeams"]
};
...
function ensureAuthorizationModel(loaded) {
  ...
  for(const name of ["teams","employeeCompensationHistory","paymentMethodCommissionHistory","financeTrialBonusStatuses"]){if(!Array.isArray(loaded[name])){loaded[name]=[];changed=true;}}
  ...
  for(const configuredStatus of loaded.statuses||[]){if(configuredStatus.partialPayment===undefined){configuredStatus.partialPayment=false;changed=true;}if(configuredStatus.isRefund===undefined){configuredStatus.isRefund=false;changed=true;}}
  const planned=(loaded.statuses||[]).find((item)=>item.systemKey==="PLANNED")||(loaded.statuses||[]).find((item)=>item.id==="st_scheduled");
  if(planned){if(!planned.systemKey){planned.systemKey="PLANNED";changed=true;}if(["Scheduled","Запланирован","Запланированное"].includes(planned.name)){planned.name="Запланированные";changed=true;}}
  let todayStatus=(loaded.statuses||[]).find((item)=>item.systemKey==="TODAY")||(loaded.statuses||[]).find((item)=>item.id==="st_today");
  if(!todayStatus){todayStatus={id:"st_today",name:"Сегодняшние",color:"#14A06F",sortOrder:0,actionType:"NONE",requiredFields:[],partialPayment:false,systemKey:"TODAY",active:true,archivedAt:null,deletedAt:null,createdAt:now(),updatedAt:now()};loaded.statuses.push(todayStatus);changed=true;}else if(!todayStatus.systemKey){todayStatus.systemKey="TODAY";changed=true;}
  for(const collectionName of ["statuses","leadSources","tags"])for(const item of loaded[collectionName]||[])if(item.deletedAt===undefined){item.deletedAt=null;changed=true;}
  const requiredSystemGrants={MANAGER:["clients.changeStatus","schedule.rescheduleTrial","schedule.assignUnassigned","analytics.view","analytics.viewManager","finance.own"],CLOSER:["analytics.view","analytics.viewCloser","finance.own"]};
...
function seedDatabase() {
  ...
  const statuses = [
    { id: "st_today", name: "Сегодняшние", color: "#14A06F", sortOrder: 0, actionType: "NONE", requiredFields: [], systemKey:"TODAY", active: true },
    { id: "st_scheduled", name: "Запланированные", color: "#4F6BED", sortOrder: 1, actionType: "NONE", requiredFields: [], systemKey:"PLANNED", active: true },
  ...
  return { meta: { version: 1, timezone: TZ, branding: { companyName: "Milton", accentColor: "#3157D5", logoUrl: "" }, reminderMinutes: 30 }, users, statuses, leadSources, tags, refusalReasons, paymentMethods, clients, availabilitySlots, trials, payments, paymentCorrections:[], notes, history, notifications: [], auditLogs: [], savedFilters: [], teams:[], employeeCompensationHistory:[], paymentMethodCommissionHistory:[], financeTrialBonusStatuses:[] };
}
...
function configPayload(type,input,current={}) {
  ...
  if(type==="statuses"){base.color=/^#[0-9a-fA-F]{6}$/.test(input.color||current.color||"")?(input.color||current.color):"#7C879E";base.actionType=current.systemKey?"NONE":(["NONE","MARK_NO_SHOW","REQUIRE_REFUSAL_REASON","REQUIRE_RESCHEDULE","REQUIRE_PAYMENT"].includes(input.actionType)?input.actionType:(current.actionType||"NONE"));base.requiredFields={NONE:[],MARK_NO_SHOW:[],REQUIRE_REFUSAL_REASON:["refusalReasonId"],REQUIRE_RESCHEDULE:["newSlotId"],REQUIRE_PAYMENT:["amount","paymentMethodId","paymentDate"]}[base.actionType];base.partialPayment=current.systemKey?false:(input.partialPayment===true||input.partialPayment==="on");base.isRefund=current.systemKey?false:(input.isRefund===true||input.isRefund==="on");}
  if(type==="tags")base.color=/^#[0-9a-fA-F]{6}$/.test(input.color||current.color||"")?(input.color||current.color):"#EDF0F5";
  return base;
}
function configUsage(type,itemId){const historical=db.history.filter((entry)=>JSON.stringify(entry.oldValue||{}).includes(itemId)||JSON.stringify(entry.newValue||{}).includes(itemId)).length;if(type==="statuses")return historical+db.clients.filter((c)=>c.currentStatusId===itemId).length+db.trials.filter((t)=>t.statusAtBookingId===itemId||t.resultStatusId===itemId).length;if(type==="leadSources")return historical+db.clients.filter((c)=>c.leadSourceId===itemId).length;if(type==="tags")return historical+db.clients.filter((c)=>(c.tagIds||[]).includes(itemId)).length;return historical;}
function systemStatus(key){return db.statuses.find((item)=>item.systemKey===key)||(key==="PLANNED"?db.statuses.find((item)=>item.id==="st_scheduled"):db.statuses.find((item)=>item.id==="st_today"));}
function operationalStatusForDate(value){const date=value?dayKey(value):null;return systemStatus(date===dayKey(now())?"TODAY":"PLANNED");}
function syncOperationalStatuses(){let changed=false;for(const c of db.clients){if(c.archivedAt||c.permanentlyDeletedAt)continue;const current=status(c.currentStatusId);if(!["TODAY","PLANNED"].includes(current?.systemKey)&&!(["st_today","st_scheduled"].includes(c.currentStatusId)))continue;const trial=db.trials.find((item)=>item.clientId===c.id&&item.active&&!item.pendingReschedule);if(!trial)continue;const desired=operationalStatusForDate(trial.preferredDate||trial.scheduledAt);if(!desired||desired.id===c.currentStatusId)continue;const changedAt=now(),oldStatusId=c.currentStatusId;c.currentStatusId=desired.id;c.statusChangedAt=changedAt;c.updatedAt=changedAt;trial.statusAtBookingId=desired.id;history(c.id,c.currentManagerId,"STATUS_CHANGED",{statusId:oldStatusId},{statusId:desired.id,automatic:true,reason:"CALENDAR_DATE"});audit(c.currentManagerId,"CLIENT",c.id,"STATUS_AUTO_CHANGED",{statusId:oldStatusId},{statusId:desired.id});changed=true;}return changed;}
function ensureOperationalNotifications(actor){
  const before=JSON.stringify(db.notifications);
  const statusChanged=syncOperationalStatuses();
  ...
  return statusChanged||before!==JSON.stringify(db.notifications);
}
...
function dashboard(actor) {
  ...
  const earned=hasPermission(actor,"finance.own")?financeReport({db,actor,hasPermission,dayKey,filters:{from:today,to:today,userId:actor.id,showRefunds:true}}).summary.salary:0;
  return { clients: visible.length, todayTrials: trials.length, completed: trials.filter((t) => t.completedAt).length, remaining: trials.filter((t) => new Date(t.scheduledAt) >= new Date()).length, payments: hasPermission(actor,"payments.view")?pays.length:0, revenue: hasPermission(actor,"payments.view")?pays.reduce((n, p) => n + Number(p.amount), 0):0, earnedToday:earned, overdue: visible.map((c)=>enrichClient(c,actor)).filter((c) => c.overdue).length, upcoming: trials.sort((a, b) => a.scheduledAt.localeCompare(b.scheduledAt)).map((t) => ({ ...t, client: enrichClient(client(t.clientId),actor) })) };
}

function compensationView(account){
  const history=db.employeeCompensationHistory.filter((row)=>row.userId===account.id).sort((a,b)=>String(b.effectiveAt).localeCompare(String(a.effectiveAt)));
  return{current:history.find((row)=>new Date(row.effectiveAt)<=new Date())||null,history:history.map((row)=>({...row,changedBy:publicUser(user(row.changedByUserId))}))};
}
function paymentMethodFinanceView(method){const history=db.paymentMethodCommissionHistory.filter((row)=>row.paymentMethodId===method.id).sort((a,b)=>String(b.effectiveAt).localeCompare(String(a.effectiveAt))),current=history.find((row)=>new Date(row.effectiveAt)<=new Date());return{...method,currentBankCommissionPercent:Number(current?.bankCommissionPercent||0),commissionHistory:history};}
function financeFilters(search){const value=(key)=>search.get(key)||"",today=dayKey(now());return{from:/^\d{4}-\d{2}-\d{2}$/.test(value("from"))?value("from"):today,to:/^\d{4}-\d{2}-\d{2}$/.test(value("to"))?value("to"):today,dateType:value("dateType"),userId:value("userId"),showRefunds:value("showRefunds")==="true"};}
function effectiveTimestamp(value){if(!value)return now();const parsed=new Date(value);return Number.isNaN(parsed.getTime())?null:parsed.toISOString();}

function bootstrapPayload(actor,requested=null) {
  const wants=(key)=>!requested||requested.has(key),result={};
  if(wants("me"))result.me={...publicUser(actor),compensation:compensationView(actor)};
  ...
  if(wants("users"))result.users=db.users.filter((u)=>hasPermission(actor,"users.view")||u.active).map((u)=>{const visible=hasPermission(actor,"users.managePermissions")?adminUserView(u):publicUser(u);if(hasPermission(actor,"finance.manageRates")||u.id===actor.id)visible.compensation=compensationView(u);return visible;});
  ...
  if(wants("paymentMethods"))result.paymentMethods=db.paymentMethods.map(paymentMethodFinanceView);
  if(wants("teams"))result.teams=hasPermission(actor,"finance.manageTeams")||hasPermission(actor,"users.view")?db.teams:[];
  if(wants("financeSettings"))result.financeSettings={trialBonusStatusIds:db.financeTrialBonusStatuses.map((row)=>row.statusId),refundStatusId:db.statuses.find((row)=>row.isRefund)?.id||null};
...
function scheduleSlotPayload(slot,actor){
  ...
  const events=(indexes.bySlot.get(slot.id)||[]).map((trial)=>{
    ...
    const crmStatus=visibleClient?status(visibleClient.currentStatusId):null;
    return{id:trial.id,clientId:mayOpen?trial.clientId:null,clientName:mayOpen?visibleClient.name:null,slotId:trial.slotId,scheduledAt:trial.scheduledAt,active:trial.active,resultStatusId:trial.resultStatusId||null,attendanceOutcome:trial.attendanceOutcome||null,statusName:crmStatus?(defaultRussianNames[crmStatus.id]||crmStatus.name):displayTrialStatus(trial),crmStatus:crmStatus?{id:crmStatus.id,name:crmStatus.name,color:crmStatus.color}:null,manager:manager?publicUser(manager):null,rescheduledTo:next?{trialId:next.id,scheduledAt:next.scheduledAt}:null};
  });
...
async function api(req, res, url) {
  ...
  if(req.method==="GET"&&url.pathname==="/api/finance"){
    if(!["finance.own","finance.team","finance.allManagers","finance.allClosers","finance.companyTurnover","finance.allPayments","finance.allTrials"].some((key)=>hasPermission(actor,key)))return fail(res,403,"Нет права на просмотр финансов");
    return json(res,200,financeReport({db,actor,hasPermission,dayKey,filters:financeFilters(url.searchParams)}));
  }
  if(req.method==="GET"&&url.pathname==="/api/export/finance.csv"){
    if(!requirePermission(res,actor,"finance.export"))return;const report=financeReport({db,actor,hasPermission,dayKey,filters:financeFilters(url.searchParams)}),rows=[["Сотрудник","Клиент","Gross","Способ оплаты","Комиссия банка %","Комиссия банка","Net","Процент сотрудника","Комиссия с продажи","Бонус за пробный","Итого зарплата","Дата оплаты","Возврат"]];
    for(const row of report.rows){const employeeId=url.searchParams.get("userId"),isCloser=Boolean(employeeId&&row.closerId===employeeId&&row.managerId!==employeeId),employee=employeeId?(isCloser?row.closerName:row.managerName):`${row.managerName} / ${row.closerName}`,rate=isCloser?row.closerRatePercent:row.managerRatePercent,earning=employeeId?(isCloser?row.closerEarning:row.managerEarning):row.totalSalary;rows.push([employee,row.clientName,row.gross,row.paymentMethod,row.bankCommissionPercent,row.bankCommission,row.net,rate,earning,0,earning,row.paymentDate,row.refund?"Был возврат":""]);}
    for(const trial of report.trials)rows.push([trial.closerName,trial.clientName,0,"Проведённый пробный",0,0,0,0,0,trial.bonus,trial.bonus,dayKey(trial.resultAt),""]);
    const csv=rows.map((row)=>row.map((value)=>`"${String(value??"").replaceAll('"','""')}"`).join(",")).join("\n");res.writeHead(200,{"Content-Type":"text/csv; charset=utf-8","Content-Disposition":"attachment; filename=milton-finance.csv"});return res.end(`\uFEFF${csv}`);
  }
  if(req.method==="GET"&&/^\/api\/finance\/clients\/[^/]+$/.test(url.pathname)){
    if(!requirePermission(res,actor,"finance.clientHistory"))return;const clientId=url.pathname.split("/").pop(),target=client(clientId);if(!target||!canOpenClient(actor,target))return fail(res,404,"Клиент не найден");const financeAccess=(candidate,key)=>key==="finance.allPayments"||key==="finance.allTrials"?true:hasPermission(candidate,key),report=financeReport({db,actor,hasPermission:financeAccess,dayKey,filters:{from:"1900-01-01",to:"2999-12-31",showRefunds:true}});return json(res,200,{payments:report.rows.filter((row)=>row.clientId===clientId),trials:report.trials.filter((row)=>row.clientId===clientId)});
  }
  if(req.method==="POST"&&url.pathname==="/api/admin/teams"){
    if(!requirePermission(res,actor,"finance.manageTeams"))return;const input=await body(req),name=String(input.name||"").trim();if(!name)return fail(res,422,"Введите название команды");if(db.teams.some((team)=>team.name.toLowerCase()===name.toLowerCase()))return fail(res,409,"Команда с таким названием уже существует");const team={id:id("team"),name,createdByUserId:actor.id,createdAt:now(),updatedAt:now()};db.teams.push(team);audit(actor.id,"TEAM",team.id,"TEAM_CREATED",null,team);saveDb();return json(res,201,team);
  }
  if(req.method==="PUT"&&/^\/api\/admin\/teams\/[^/]+$/.test(url.pathname)){
    if(!requirePermission(res,actor,"finance.manageTeams"))return;const team=db.teams.find((row)=>row.id===url.pathname.split("/").pop());if(!team)return fail(res,404,"Команда не найдена");const input=await body(req),name=String(input.name||"").trim();if(!name)return fail(res,422,"Введите название команды");if(db.teams.some((row)=>row.id!==team.id&&row.name.toLowerCase()===name.toLowerCase()))return fail(res,409,"Команда с таким названием уже существует");const oldValue={...team};team.name=name;team.updatedAt=now();audit(actor.id,"TEAM",team.id,"TEAM_UPDATED",oldValue,team);saveDb();return json(res,200,team);
  }
  if(req.method==="DELETE"&&/^\/api\/admin\/teams\/[^/]+$/.test(url.pathname)){
    if(!requirePermission(res,actor,"finance.manageTeams"))return;const teamId=url.pathname.split("/").pop();if(db.users.some((account)=>account.teamId===teamId))return fail(res,409,"Сначала удалите сотрудников из команды");const team=db.teams.find((row)=>row.id===teamId);if(!team)return fail(res,404,"Команда не найдена");db.teams=db.teams.filter((row)=>row.id!==teamId);audit(actor.id,"TEAM",team.id,"TEAM_DELETED",team,null);saveDb();return json(res,200,{id:team.id,deleted:true});
  }
  if(req.method==="PUT"&&/^\/api\/admin\/users\/[^/]+\/compensation$/.test(url.pathname)){
    if(!requirePermission(res,actor,"finance.manageRates"))return;const target=user(url.pathname.split("/")[4]);if(!target)return fail(res,404,"Сотрудник не найден");const input=await body(req),sales=Number(input.salesCommissionPercent),trial=Number(input.conductedTrialAmount||0),effectiveAt=effectiveTimestamp(input.effectiveAt);if(!effectiveAt||!Number.isFinite(sales)||sales<0||sales>100||!Number.isFinite(trial)||trial<0||trial>1000000)return fail(res,422,"Укажите процент от 0 до 100, дату и корректную сумму за пробный");const previous=compensationView(target).current,row={id:id("rate"),userId:target.id,salesCommissionPercent:sales,conductedTrialAmount:target.role==="CLOSER"?trial:0,effectiveAt,changedByUserId:actor.id,createdAt:now()};db.employeeCompensationHistory.push(row);audit(actor.id,"USER",target.id,"COMPENSATION_CHANGED",previous,row);saveDb();return json(res,200,compensationView(target));
  }
  if(req.method==="PUT"&&/^\/api\/admin\/payment-methods\/[^/]+\/commission$/.test(url.pathname)){
    if(!requirePermission(res,actor,"finance.manageRates"))return;const method=db.paymentMethods.find((row)=>row.id===url.pathname.split("/")[4]);if(!method)return fail(res,404,"Способ оплаты не найден");const input=await body(req),percent=Number(input.bankCommissionPercent),effectiveAt=effectiveTimestamp(input.effectiveAt);if(!effectiveAt||!Number.isFinite(percent)||percent<0||percent>100)return fail(res,422,"Комиссия банка должна быть от 0 до 100% и иметь корректную дату");const oldValue=paymentMethodFinanceView(method),row={id:id("bankrate"),paymentMethodId:method.id,bankCommissionPercent:percent,effectiveAt,changedByUserId:actor.id,createdAt:now()};db.paymentMethodCommissionHistory.push(row);audit(actor.id,"PAYMENT_METHOD",method.id,"BANK_COMMISSION_CHANGED",oldValue,row);saveDb();return json(res,200,paymentMethodFinanceView(method));
  }
  if(req.method==="PUT"&&url.pathname==="/api/admin/finance-settings"){
    if(!requirePermission(res,actor,"finance.manageRates"))return;const input=await body(req),ids=[...new Set(Array.isArray(input.trialBonusStatusIds)?input.trialBonusStatusIds:[])],refundStatusId=input.refundStatusId||null;if(ids.some((statusId)=>!db.statuses.some((row)=>row.id===statusId))||(refundStatusId&&!db.statuses.some((row)=>row.id===refundStatusId)))return fail(res,422,"Выбран неизвестный статус");const oldValue={trialBonusStatusIds:db.financeTrialBonusStatuses.map((row)=>row.statusId),refundStatusId:db.statuses.find((row)=>row.isRefund)?.id||null};db.financeTrialBonusStatuses=ids.map((statusId)=>({statusId,configuredByUserId:actor.id,configuredAt:now()}));for(const configured of db.statuses)configured.isRefund=configured.id===refundStatusId;audit(actor.id,"FINANCE_SETTINGS","finance-statuses","FINANCE_STATUSES_CHANGED",oldValue,{trialBonusStatusIds:ids,refundStatusId});saveDb();return json(res,200,{trialBonusStatusIds:ids,refundStatusId});
  }
  ...
  if (/^\/api\/admin\/config\/[^/]+(?:\/[^/]+)?(?:\/(?:archive|restore))?$/.test(url.pathname)) {
    ...
    if(req.method==="PUT"&&!action){const input=await body(req),next=configPayload(type,input,target);if(!next)return fail(res,422,"Введите название");const oldValue=structuredClone(target);Object.assign(target,next,{updatedAt:now()});audit(actor.id,"CONFIG",target.id,"CONFIG_UPDATED",oldValue,{type,...target});saveDb();return json(res,200,target);}
    if(req.method==="DELETE"&&!action){if(!["statuses","leadSources","tags"].includes(type))return fail(res,405,"Удаление этого справочника недоступно");if(type==="statuses"&&target.systemKey)return fail(res,409,"Системные статусы «Сегодняшние» и «Запланированные» нельзя удалить, но можно переименовать");const input=await body(req),usageCount=configUsage(type,target.id);if(usageCount&&!input.confirmed)return fail(res,409,`Значение используется в ${usageCount} записях. История и клиенты будут сохранены. Подтвердите удаление.`,{requiresConfirmation:true,usageCount});const oldValue=structuredClone(target);if(usageCount){target.deletedAt=now();target.active=false;target.archivedAt=target.archivedAt||target.deletedAt;}else db[cfg.collection]=collection.filter((item)=>item.id!==target.id);audit(actor.id,"CONFIG",target.id,"CONFIG_DELETED",oldValue,{type,usageCount,historicalRecordPreserved:Boolean(usageCount)});saveDb();return json(res,200,{id:target.id,deleted:true,usageCount,historicalRecordPreserved:Boolean(usageCount)});}
  ...
  if (req.method === "POST" && url.pathname === "/api/clients") {
    ...
    const managerId = actor.role === "MANAGER" ? actor.id : input.managerId,initialOperationalStatus=operationalStatusForDate(input.preferredDate||slot?.startAt); const c = { id: id("cl"), name: input.name.trim(), normalizedPhone, originalPhone: input.phone, originalManagerId: managerId, currentManagerId: managerId, currentCloserId: unassigned?null:input.closerId, currentStatusId: initialOperationalStatus?.id || input.statusId || db.statuses.filter((s) => s.active&&!s.deletedAt).sort((a,b) => a.sortOrder-b.sortOrder)[0].id, leadSourceId: input.leadSourceId || null, tagIds: input.tagIds || [], registrationComment: input.comment || "",archivedAt:null,archivedByUserId:null,archiveReason:null,permanentlyDeletedAt:null,permanentlyDeletedByUserId:null,statusChangedAt:now(), createdAt: now(), updatedAt: now() };
  ...
  if (req.method === "DELETE" && /^\/api\/payments\/[^/]+$/.test(url.pathname)) {
    if(!requirePermission(res,actor,"payments.delete"))return;const payment=db.payments.find((p)=>p.id===url.pathname.split("/")[3]&&!p.voidedAt);if(!payment)return fail(res,404,"Активная оплата не найдена");const c=client(payment.clientId);if(!c||c.archivedAt||c.permanentlyDeletedAt)return fail(res,409,"Оплаты архивного клиента доступны только для просмотра");if(!clientMatchesScope(actor,c,"payments"))return fail(res,403,"Оплата вне вашей области данных");const input=await body(req);if(input.confirmed!==true)return fail(res,422,"Подтвердите удаление оплаты");const deletedAt=now(),snapshot={id:payment.id,clientId:payment.clientId,amount:payment.amount,paymentMethodId:payment.paymentMethodId,paymentDate:payment.paymentDate,comment:payment.comment};payment.voidedAt=deletedAt;payment.updatedAt=deletedAt;c.updatedAt=deletedAt;history(c.id,actor.id,"PAYMENT_DELETED",snapshot,{voidedAt:deletedAt});audit(actor.id,"PAYMENT",payment.id,"PAYMENT_DELETED",snapshot,{voidedAt:deletedAt});saveDb();return json(res,200,{id:payment.id,deleted:true,voidedAt:deletedAt});
  }
